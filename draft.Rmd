...
```{r}
trans_ent = read_csv("NYC_Transit_Subway_Entrance_And_Exit_Data.csv",
...
## Problem 2
### Manipute the Mr.Trash Wheel sheet
```{r}
data_mr = read_excel("Trash Wheel Collection Data.xlsx") %>% 
  janitor::clean_names() %>% 
  drop_na(dumpster) %>% 
  mutate(group = "mr")
...
```{r}
data_pro = read_excel("Trash Wheel Collection Data.xlsx",2) %>% 
  janitor::clean_names() %>% 
  drop_na(dumpster) %>% 
...
# data_pro = janitor::clean_names(data_pro)
# data_pro = drop_na(data_pro, dumpster)
# data_pro = add_column(data_pro, sports_balls = 0, .after = "chip_bags")
# data_pro = mutate(data_pro, group = "professor")
# data_pro[,3] = as.character(unlist(data_pro[,3]))
# data_trash =
#   bind_rows(data_mr, data_pro) %>%
#   janitor::clean_names() %>%
